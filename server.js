...
const hasGithubStore = Boolean(GITHUB_TOKEN && GITHUB_REPO && GITHUB_DATA_PATH);
let githubSha = null;
...
async function loadStore() {
  if (!hasGithubStore) return loadStoreLocal();
  try {
    const url = `https://api.github.com/repos/${GITHUB_REPO}/contents/${encodeURIComponent(GITHUB_DATA_PATH).replace(/%2F/g, "/")}?ref=${encodeURIComponent(GITHUB_BRANCH)}`;
    ...
    return loadStoreLocal();
  }
}

async function saveStore() {
  const raw = JSON.stringify(keys, null, 2);
  fs.writeFileSync(STORE_PATH, raw, "utf8");
  if (!hasGithubStore) return;
  const pathPart = encodeURIComponent(GITHUB_DATA_PATH).replace(/%2F/g, "/");
  const url = `https://api.github.com/repos/${GITHUB_REPO}/contents/${pathPart}`;
  let sha = githubSha;
  try {
    if (!sha) {
      const existing = await githubRequest(`${url}?ref=${encodeURIComponent(GITHUB_BRANCH)}`);
      if (existing.ok) {
    ...
    const payload = {
      message: `update ${GITHUB_DATA_PATH}`,
    ...
    const data = await res.json();
    githubSha = data.content?.sha || sha || null;
  } catch (e) {
    console.error("saveStore error:", e.message || e);
    throw e;
  }
}
      ...
      <div class="content">
        ...
        <div id="panelView" class="hidden">
          ...
          <div id="tab5" class="tabPane">
            <div class="grid2">
              <a class="socialBtn" href="${FF_URL}" target="_blank" rel="noopener noreferrer">🎮 <span>Free Fire</span></a>
              <a class="socialBtn" href="${FF_MAX_URL}" target="_blank" rel="noopener noreferrer">🔥 <span>FF MAX</span></a>
            </div>
...
app.post("/api/check", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Không lưu được key" });
  }
...
async function bootstrap() {
  keys = await loadStore();
  normalizeAllKeys();
  try {
    await saveStore();
  } catch (e) {
    console.error("init save warning:", e.message || e);
  }

  app.listen(PORT, () => {
    console.log(`ATH server running on http://localhost:${PORT}`);
  });
}
